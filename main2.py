...
async def main():
    ...
    settlements_houses_count = {}  # Словарь для хранения количества домов для каждого settlement
    ...
    async with aiohttp.ClientSession() as session:
        for setl in tqdm(list_of_all):
            ...
            # Считаем количество домов для каждого settlement
            total_houses = 0
            for result in results:
                if result:
                    total_houses += len(result)  # Увеличиваем счетчик домов для текущего settlement
            
            settlements_houses_count[setl] = total_houses  # Записываем количество домов для текущего settlement

    # Выводим результат
    for settlement, house_count in settlements_houses_count.items():
        print(f"{settlement}: {house_count} домов")
